...
# CONFIGURATION
# ==========================================
# We assume the tester is inside your minishell folder.
# So the makefile is one level up (../)
export MINISHELL_PATH="../"
export EXECUTABLE=minishell
# We set RUNDIR to current directory so it finds the tests files relative to this script
RUNDIR="."
...
main() {
	...
	if [[ ! -f "$MINISHELL_PATH/$EXECUTABLE" ]] ; then
		...
		# Added quotes and -C flag validation
		make -C "$MINISHELL_PATH"
		
		if [[ ! -f "$MINISHELL_PATH/$EXECUTABLE" ]] ; then
			echo -e "\033[1;31mCOMPILING FAILED\033[m" 
			echo "Could not find executable at: $MINISHELL_PATH/$EXECUTABLE"
			exit 1
		fi
	fi
